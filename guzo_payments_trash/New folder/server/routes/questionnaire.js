...
router.get("/export/excel/by-subcity", authMiddleware, async (req, res) => {
  try {
    ...
    const grouped = rows.reduce((acc, row) => {
      const key = (row.subCity || "Unknown").trim() || "Unknown";
      if (!acc[key]) acc[key] = [];
      acc[key].push(row);
      return acc;
    }, {});

    Object.entries(grouped).forEach(([subCity, items]) => {
      const safeSheetName = subCity.substring(0, 31) || "Unknown";
      const sheet = workbook.addWorksheet(safeSheetName);
      addQuestionnaireSheet(sheet, items);
    });
