...
//     return new Set(
//       rows.map((r) => (r.organization || "").trim()).filter(Boolean),
//     ).size;
//   }, [rows]);

//   const totalSubCities = useMemo(() => {
//     return new Set(rows.map((r) => (r.subCity || "").trim()).filter(Boolean))
//       .size;
//   }, [rows]);
...
//     const map = {};
//     rows.forEach((r) => {
//       const key = (r.nearChurch || "N/A").trim() || "N/A";
//       map[key] = (map[key] || 0) + 1;
//     });
...
//     const map = {};
//     rows.forEach((r) => {
//       const key = (r.subCity || "N/A").trim() || "N/A";
//       map[key] = (map[key] || 0) + 1;
//     });
...
//     const map = {};
//     rows.forEach((r) => {
//       const key = (r.organization || "N/A").trim() || "N/A";
//       map[key] = (map[key] || 0) + 1;
//     });
...
//     rows.forEach((r) => {
//       const org = (r.organization || "N/A").trim() || "N/A";
...
//       if (r.subCity) map[org].subCities.add(r.subCity);
//       if (r.nearChurch) map[org].nearChurches.add(r.nearChurch);
//     });
...
//       const key = (item.organization || "N/A").trim() || "N/A";
//       if (!acc[key]) {
//         acc[key] = {
//           organization: key,
//           rows: [],
...
//     try {
//       const data = group.rows.map((r, index) => ({
//         No: index + 1,
...
//       doc.setFontSize(16);
//       doc.text(`Organization: ${group.organization}`, 14, 18);
...
//         <h1 className="text-lg md:text-4xl font-bold text-purple-600 mb-4">
