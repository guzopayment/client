        ...
        <main className="flex-1 min-w-0 px-2 py-3 sm:px-3 sm:py-4 md:p-8 pt-14 sm:pt-16 md:pt-8">
          ...
          <div className="bg-white rounded-2xl shadow p-4 md:p-5 mb-8 hover:shadow-2xl hover:-translate-y-1 transition">
            ...
            {organizationSummaryRows.length > 0 && (
              <Pagination
                page={orgTablePage}
                totalPages={totalOrgTablePages}
                onChange={setOrgTablePage}
              />
            )}
          </div>
          ...
          <div className="space-y-6 md:space-y-8">
                ...
                >
                  ...
                  {group.rows.length > 0 && (
                    <Pagination
                      ...
                      onChange={(nextPage) =>
                        setGroupPage(group.organization, nextPage)
                      }
                    />
                  )}
                </div>
            ...
            {groupedByOrganizationList.length > 0 && (
              <Pagination
                page={orgGroupPage}
                totalPages={totalOrgGroupPages}
                onChange={setOrgGroupPage}
              />
            )}
          </div>
...
function buildPages(currentPage, totalPages, boundaryCount = 2, siblingCount = 1) {
  ...
  for (let i = Math.max(totalPages - boundaryCount + 1, 1); i <= totalPages; i += 1) {
    pages.add(i);
  }
