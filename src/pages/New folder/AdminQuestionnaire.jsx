...
export default function AdminQuestionnaire() {
  ...
  const exportGroupExcel = async (group) => {
    try {
      ...
      const safeName = `${group.subCity || "NA"}-${group.woreda || "NA"}-${group.nearChurch || "NA"}`
        .replace(/[\/:*?"<>|]/g, "_")
        .slice(0, 120);
  ...
  const exportGroupPDF = async (group) => {
    try {
      ...
      doc.setFontSize(10);
      doc.text(`Woreda: ${group.woreda || "N/A"} | Near Church: ${group.nearChurch || "N/A"}`, 14, 22);
              ...
              return (
                ...
                >
                  ...
                  <Pagination
                    page={currentInnerPage}
                    totalPages={totalInnerPages}
                    onChange={(nextPage) => setGroupedListRowPage(group, nextPage)}
                  />
                </div>
              );
            })}
