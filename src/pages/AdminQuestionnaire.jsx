...
export default function AdminQuestionnaire() {
  ...
  // grouped by sub-city, woreda and near-church block pagination
  const [groupedListPage, setGroupedListPage] = useState(1);
  const groupedListPerPage = 5;

  // inner row pagination for each grouped list block
  const [groupedListRowPages, setGroupedListRowPages] = useState({});
  const groupedListRowsPerPage = 8;
  ...
  useEffect(() => {
    setGroupedListPage(1);
    setGroupedListRowPages({});
  }, [search, subCityFilter, rows]);

  const totalGroupedListPages =
    Math.ceil(groupedList.length / groupedListPerPage) || 1;

  useEffect(() => {
    if (groupedListPage > totalGroupedListPages) {
      setGroupedListPage(totalGroupedListPages);
    }
  }, [groupedListPage, totalGroupedListPages]);

  const paginatedGroupedList = useMemo(() => {
    const start = (groupedListPage - 1) * groupedListPerPage;
    return groupedList.slice(start, start + groupedListPerPage);
  }, [groupedList, groupedListPage]);

  const getGroupedListRowPageKey = (group) =>
    `${group.subCity || "N/A"}__${group.woreda || "N/A"}__${group.nearChurch || "N/A"}`;

  const getGroupedListRowPage = (group) =>
    groupedListRowPages[getGroupedListRowPageKey(group)] || 1;

  const setGroupedListRowPage = (group, nextPage) => {
    const key = getGroupedListRowPageKey(group);
    setGroupedListRowPages((prev) => ({
      ...prev,
      [key]: nextPage,
    }));
  };

  const getGroupedListRowTotalPages = (group) =>
    Math.ceil(group.rows.length / groupedListRowsPerPage) || 1;

  const getPaginatedGroupedListRows = (group) => {
    const currentPage = getGroupedListRowPage(group);
    const start = (currentPage - 1) * groupedListRowsPerPage;
    return group.rows.slice(start, start + groupedListRowsPerPage);
  };
  ...
  const buildGroupExportRows = (group) =>
  ...
  const exportGroupExcel = async (group) => {
    try {
      const rowsForExport = buildGroupExportRows(group);
      ...
      const safeName =
        `${group.subCity || "NA"}-${group.woreda || "NA"}-${group.nearChurch || "NA"}`
          .replace(/[\/:*?"<>|]/g, "_")
          .slice(0, 120);
      ...
      showModal("Export Error", "Failed to export group Excel", "error");
    }
  };
  ...
  const exportGroupPDF = async (group) => {
    try {
      ...
      doc.setFontSize(16);
      doc.text(`${group.subCity || "N/A"}`, 14, 16);
      doc.setFontSize(10);
      doc.text(
        `Woreda: ${group.woreda || "N/A"} | Near Church: ${group.nearChurch || "N/A"}`,
        14,
        22,
      );
      doc.text(`Total Records: ${(group.rows || []).length}`, 14, 28);
      ...
      autoTable(doc, {
        ...
        headStyles: { fillColor: [196, 181, 253], textColor: [0, 0, 0] },
        theme: "grid",
      });
      ...
      showModal("PDF Export Error", "Failed to export group PDF", "error");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 w-full overflow-x-hidden">
      <TopTicker
        ...
        items={[
          "አዲስ የመጠይቅ ምዝገባዎች ገጹ Refresh ካደረጉት በኋላ ይታያሉ",
      ...
      <div className="flex min-h-screen w-full pt-[28px] sm:pt-[32px] md:pt-[38px]">
        ...
        <aside
          className={`fixed md:sticky md:top-0 md:self-start z-40 h-full md:h-screen overflow-y-auto w-64 bg-purple-400 text-white pt-16 md:pt-6 p-6 shadow-xl
        ...
        <main
          className="flex-1 min-w-0 px-2 py-3 sm:px-3 sm:py-4 md:p-8 pt-14 sm:pt-16 md:pt-8"
          overflow-y-scroll
        >
          ...
          <div className="bg-white rounded-2xl shadow p-4 md:p-5 mb-8 hover:shadow-2xl hover:-translate-y-1 transition">
            <h1 className="text-lg md:text-4xl font-bold text-purple-600">
              Table showing the most 10(ten) recently submitted data
            </h1>
            ...
            <div className="overflow-x-auto overscroll-x-contain mb-8 rounded-xl shadow bg-white">
              <table className="min-w-[760px] w-max bg-white rounded-xl border-separate border-spacing-0">
                <thead className="bg-purple-400 text-white">
                  <tr>
                    <th className="px-1.5 py-2 sm:px-2 sm:py-2.5 text-[9px] sm:text-[10px] md:text-sm whitespace-nowrap">
                      Questionnaire ID
                    </th>
                    <th className="px-1.5 py-2 sm:px-2 sm:py-2.5 text-[9px] sm:text-[10px] md:text-sm whitespace-nowrap">
                      Full Name
                  ...
                  {filteredRows.length === 0 ? (
                    ...
                    paginated.map((item) => (
                      ...
                      >
                        ...
                        <td className="px-1.5 py-2 sm:px-2 sm:py-3 text-[9px] sm:text-[10px] md:text-sm font-semibold text-purple-700 whitespace-nowrap align-top">
                          {item.questionnaireId || "—"}
                        </td>

                        {/* <td className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap"> */}
                        <td className="px-1.5 py-2 sm:px-2 sm:py-3 text-[9px] sm:text-[10px] md:text-sm leading-tight whitespace-normal break-words align-top min-w-[90px]">
                          {editingId === item._id ? (
                        ...
                        {/* <td className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap"> */}
                        <td className="px-1.5 py-2 sm:px-2 sm:py-3 text-[9px] sm:text-[10px] md:text-sm whitespace-nowrap align-top">
                          {editingId === item._id ? (
                            <input
                        ...
                        {/* <td className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap"> */}
                        <td className="px-1.5 py-2 sm:px-2 sm:py-3 text-[9px] sm:text-[10px] md:text-sm leading-tight whitespace-normal break-words align-top min-w-[92px]">
                          {editingId === item._id ? (
                            <input
                        ...
                        {/* <td className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap"> */}
                        <td className="px-1.5 py-2 sm:px-2 sm:py-3 text-[9px] sm:text-[10px] md:text-sm leading-tight whitespace-normal break-words align-top min-w-[70px]">
                          {editingId === item._id ? (
                            <input
                        ...
                        <td className="px-1 py-2 sm:px-2 sm:py-3 align-top">
                          {editingId === item._id ? (
                            ...
                            <div className="flex flex-wrap justify-center gap-2">
                              <button
                                ...
                                className="bg-blue-600 text-white px-1.5 py-1 sm:px-2 sm:py-1 rounded hover:bg-blue-700 transition text-[9px] sm:text-[10px] md:text-sm leading-none whitespace-nowrap"
                              >
                                Edit
                              </button>
                              <button
                                ...
                                className="bg-red-600 text-white px-1.5 py-1 sm:px-2 sm:py-1 rounded hover:bg-red-700 transition text-[9px] sm:text-[10px] md:text-sm leading-none whitespace-nowrap"
                              >
                                Delete
                              </button>
                            </div>
          ...
          <div className="space-y-6 mb-8">
              ...
              return (
                ...
                >
                  ...
                  <div className="overflow-x-auto overscroll-x-contain rounded-xl border">
                    <table className="min-w-[560px] w-max bg-white rounded-xl border-separate border-spacing-0">
                      <thead className="bg-purple-100">
                        <tr>
                          ...
                          {/* <th className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap"> */}
                          <th className="px-1.5 py-2 sm:px-2 sm:py-2.5 text-[9px] sm:text-[10px] md:text-sm leading-tight whitespace-normal break-words">
                            Phone
                          </th>
                          {/* <th className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap"> */}
                          <th className="px-1.5 py-2 sm:px-2 sm:py-2.5 text-[9px] sm:text-[10px] md:text-sm leading-tight whitespace-normal break-words">
                  ...
                  {group.rows.length > 0 && (
                    <Pagination
                      page={currentInnerPage}
                      totalPages={totalInnerPages}
                      onChange={(nextPage) =>
                        setSubCityRowPage(group.subCity, nextPage)
                      }
                    />
                  )}
                </div>
            ...
            {groupedBySubCityList.length > 0 && (
              <Pagination
                page={subCityGroupPage}
                totalPages={totalSubCityGroupPages}
                onChange={setSubCityGroupPage}
              />
            )}
          </div>
          ...
          <div className="space-y-6 md:space-y-8">
            {paginatedGroupedList.map((group, idx) => {
              const currentInnerPage = getGroupedListRowPage(group);
              const totalInnerPages = getGroupedListRowTotalPages(group);
              const visibleRows = getPaginatedGroupedListRows(group);
              ...
              return (
                ...
                >
                  ...
                  <div className="overflow-x-auto overscroll-x-contain rounded-xl border">
                    <table className="min-w-[720px] w-max bg-white rounded-xl border-separate border-spacing-0">
                      <thead className="bg-purple-100">
                        <tr>
                          <th className="px-2 py-2 text-[11px] md:text-sm whitespace-nowrap">
                            Questionnaire ID
                  ...
                  <Pagination
                    ...
                    totalPages={totalInnerPages}
                    onChange={(nextPage) =>
                      setGroupedListRowPage(group, nextPage)
                    }
                  />
                </div>
              );
            ...
            {groupedList.length > 0 && (
              <Pagination
                ...
                totalPages={totalGroupedListPages}
                onChange={setGroupedListPage}
              />
            )}
          </div>
...
function Pagination({ page, totalPages, onChange, className = "" }) {
  ...
  return (
    <div
      className={`flex flex-wrap justify-center items-center gap-2 mt-6 ${className}`}
    >
      <button
        disabled={page === 1}
        onClick={() => goTo(page - 1)}
        className="bg-purple-600 text-white px-4 py-2 rounded-full shadow disabled:opacity-40 hover:bg-purple-700 transition"
      >
        Prev
      </button>
      ...
      {pages.map((item, index) =>
        item === "..." ? (
          <span
            key={`ellipsis-${index}`}
            className="px-2 text-purple-700 font-semibold"
          >
            ...
          </span>
        ) : (
          <button
            key={item}
            onClick={() => goTo(item)}
            className={`min-w-[42px] px-3 py-2 rounded-full shadow transition font-semibold ${
              item === page
...
function MetricCard({ label, value, className = "" }) {
  return (
    <div
      className={`min-w-0 w-full bg-white rounded-xl sm:rounded-2xl shadow-lg p-2 sm:p-3 md:p-5 hover:shadow-2xl transition ${className}`}
    >
      <p className="text-[8px] sm:text-[10px] md:text-sm text-gray-500 mb-1 leading-tight truncate">
        {label}
      </p>
